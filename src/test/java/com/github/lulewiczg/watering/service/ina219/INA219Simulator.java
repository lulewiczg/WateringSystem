 ...
 */
public class INA219Simulator implements INA219Register {
    private int configurationRegister;
    private int calibrationRegister;
    private final double shuntVoltage;
    private final double busVoltage;

    /**
     * Constructs a new INA219 simulator using the provided voltage values.
     *
     * @param shuntVoltage The voltage present across the shunt registor to be simulated.
     * @param busVoltage   The bus voltage to be simulated.
     */
    public INA219Simulator(double shuntVoltage, double busVoltage) {
     ...
     * {@inheritDoc}
     */
    public void writeRegister(RegisterAddress ra, int value) {
        switch (ra) {
            case CONFIGURATION:
     ...
     * {@inheritDoc}
     */
    public short readSignedRegister(RegisterAddress ra) {
        switch (ra) {
            case CONFIGURATION:
     ...
     * {@inheritDoc}
     */
    public int readRegister(RegisterAddress ra) {
    ...
    private int calculateCurrentRegister() {
        return (calculateShuntVoltage() * calibrationRegister) / 4096;
    }
}
